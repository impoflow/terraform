...
resource "aws_instance" "scrapper_instance" {
  ...
  user_data = <<-EOF
              ...
              docker run -d -name scrapper -e "BUCKET_NAME=${var.bucket-name}" -e "REGION=us-east-1" -e "USERS=300" -e "URL=https://github.com/search?q=todo+list+java&type=repositories&p=" ${var.docker-username}/github-scrapper
              sleep 120
              docker stop scrapper
              docker container prune
